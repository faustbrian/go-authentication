...
`go-authentication` owns authentication. `go-authorization` consumes authenticated
principals to make access decisions. `go-service` composes authentication
...
- Secret-safe `go-log` integration.
- Authentication metrics and traces through optional `go-telemetry` adapters.
...
- No custom cryptographic algorithms, JWT implementation, or password hashing
  primitive.
- No outbound OAuth2 token acquisition; that remains in `go-http-client`.
...
- Authentication-versus-authorization boundary guide with `go-authorization`
...
- Authentication composes with `go-service` and `go-authorization` without
  dependency cycles.
