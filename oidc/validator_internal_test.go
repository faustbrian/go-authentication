...
func TestJSONUnicodeValidationMatrix(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "ordinary escape before invalid surrogate", encoded: []byte(`{"x":"\n\uD800"}`), valid: false},
		{name: "exact Unicode escape length", encoded: []byte{'"', '\\', 'u', '0', '0', '4', '1'}, valid: true},
		{name: "valid escape before invalid surrogate", encoded: []byte(`{"x":"\u0041\uD800"}`), valid: false},
		...
		{name: "surrogate pair at exact suffix", encoded: []byte(`"\uD800\uDC00`), valid: true},
		{name: "minimum surrogate pair", encoded: []byte(`{"x":"\uD800\uDC00"}`), valid: true},
		{name: "maximum surrogate pair", encoded: []byte(`{"x":"\uDBFF\uDFFF"}`), valid: true},
		{name: "below high surrogate range", encoded: []byte(`{"x":"\uD7FF"}`), valid: true},
		{name: "above low surrogate range", encoded: []byte(`{"x":"\uE000"}`), valid: true},
		{name: "invalid low surrogate digits", encoded: []byte(`{"x":"\uD800\uZZZZ"}`), valid: false},
		{name: "invalid low escape marker", encoded: []byte(`{"x":"\uD800\xDC00"}`), valid: false},
		{name: "truncated low surrogate", encoded: []byte{'"', '\\', 'u', 'D', '8', '0', '0', '\\', 'u', 'D', 'C'}, valid: false},
		{name: "low surrogate below range", encoded: []byte(`{"x":"\uD800\uDBFF"}`), valid: false},
		{name: "low surrogate above range", encoded: []byte(`{"x":"\uD800\uE000"}`), valid: false},
		{name: "non-low surrogate pair", encoded: []byte(`{"x":"\uD800\u0041"}`), valid: false},
		{name: "unpaired low surrogate", encoded: []byte(`{"x":"\uDC00"}`), valid: false},
		{name: "unpaired maximum low surrogate", encoded: []byte(`{"x":"\uDFFF"}`), valid: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
