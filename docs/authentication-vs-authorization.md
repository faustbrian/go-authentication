...
`go-authentication` owns credential syntax, verification, issuer trust,
...
`go-authorization` consumes the immutable principal and domain resource.
`go-service` may compose both middleware layers but must not reimplement
