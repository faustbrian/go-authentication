...
4. Wrap the handler with `authenticationhttp.NewMiddleware`.
...
For an opaque service token, combine `authenticationhttp.BearerAuthorization`,
`bearer.ValidatorFunc`, and `authenticationhttp.NewMiddleware`. The compiled implementation
is `ExampleNewMiddleware` in `adapters/http/example_test.go`.
...
latency, fixed diagnostic messages, and either `adapters/slog` or `adapters/otel`.
