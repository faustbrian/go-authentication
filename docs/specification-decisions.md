...
| Executable evidence | `authenticationhttp.TestBasicAuthorizationExtractionIsStrict`, `authenticationhttp.TestBearerAuthorizationExtractionEnforcesGrammarAndBounds`, `authenticationhttp.FuzzAuthorizationExtraction`, and `authenticationhttp.FuzzCredentialHeaderSet` |
| Public surface | `authhttp.BasicAuthorization`, `authhttp.BearerAuthorization`, `authhttp.Extractor`, and `FailureInvalid` versus `FailureAbsent`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestRFC7617BasicCredentialVectors`, `authenticationhttp.TestBasicAuthorizationExtractionIsStrict`, `authenticationhttp.TestBasicAuthorizationPreservesOctetsAndPasswordColons`, and Basic extraction fuzzing |
| Public surface | `authhttp.BasicAuthorization`, `authentication.BasicCredential`, `basic.NewStatic`, and `authentication.NewChallenge`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestRFC6750BearerHeaderVector`, `authenticationhttp.TestBearerAuthorizationExtractionEnforcesGrammarAndBounds`, `authenticationhttp.TestBearerAuthorizationPipeRequiresExplicitOptIn`, and `authenticationhttp.TestPrivateTokenAndNameBoundaries` |
| Public surface | `authhttp.BearerAuthorization`, `authhttp.BearerQuery`, `authhttp.BearerCookie`, `authhttp.WithBearerPipe`, and `authhttp.WithBearerMaxBytes`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestBearerQueryAndCookieAreExplicitSources`, `authenticationhttp.TestNamedBearerSourcesRejectAbsentDuplicateAndHostileValues`, `authenticationhttp.FuzzBearerQueryExtraction`, and `authenticationhttp.TestMiddlewareAuthenticatesWithoutReadingBodyOrWrappingWriter` |
| Public surface | `authhttp.BearerAuthorization`, `authhttp.BearerQuery`, `authhttp.BearerCookie`, `authhttp.Extractor`, and `authhttp.NewMiddleware`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestAPIKeySourcesMustBeExplicitAndRejectDuplicates`, `authenticationhttp.TestAPIKeySourcesRejectMalformedQueriesAndBoundedValues`, `apikey.TestStaticAuthenticatesKeyByDeterministicID`, `apikey.TestStaticRotationAtomicallyReplacesActiveKeys`, and `apikey.TestStaticRejectsDuplicateKeyConfiguration` |
| Public surface | `authhttp.APIKeyHeader`, `authhttp.APIKeyQuery`, `authhttp.APIKeyCookie`, `authentication.APIKeyCredential`, `apikey.New`, and `apikey.NewStatic`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestBasicAuthorizationExtractionIsStrict`, `authenticationhttp.TestAPIKeySourcesMustBeExplicitAndRejectDuplicates`, `authenticationhttp.TestExtractorSeparatesOriginAndProxyCredentials`, and `authenticationhttp.FuzzCredentialHeaderSet` |
| Public surface | `authhttp.Extractor`, every built-in `Source`, `ErrAmbiguousCredentials`, and `ErrCredentialsInvalid`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestFormatChallengeSortsAndEscapesParameters`, `authenticationhttp.FuzzChallengeFormatting`, `authenticationhttp.TestMiddlewareFailsClosedWithChallengesAndRedaction`, `authenticationhttp.TestMiddlewareUsesChallengeFromFailure`, and `authenticationhttp.TestMiddlewareTreatsMissingOrInvalidFailureChallengesAsUnavailable` |
| Public surface | `authentication.NewChallenge`, `authhttp.FormatChallenge`, `authhttp.WithChallenges`, `authhttp.NewMiddleware`, and HTTP status/header behavior; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestMiddlewareFailsClosedWithChallengesAndRedaction`, `authenticationhttp.TestOptionalMiddlewareAllowsOnlyAbsentCredentials`, `authenticationhttp.TestMiddlewareRejectsInvalidConfigurationAndResult`, `authenticationhttp.TestMiddlewarePropagatesRequestCancellation`, and `authenticationhttp.TestMiddlewareTreatsMissingOrInvalidFailureChallengesAsUnavailable` |
| Public surface | `FailureKind`, exported sentinel errors, `authhttp.WithOptionalAnonymous`, `authhttp.NewMiddleware`, and HTTP response behavior; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestBearerSourcesRejectMultipleTransmissionMethods` |
| Public surface | `authhttp.Extractor`, `authhttp.BearerAuthorization`, and `authhttp.BearerQuery("access_token")`; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
...
| Executable evidence | `authenticationhttp.TestMiddlewareFailsClosedWithChallengesAndRedaction`, `authenticationhttp.TestMiddlewareUsesChallengeFromFailure`, `authenticationhttp.TestMiddlewareTreatsMissingOrInvalidFailureChallengesAsUnavailable` |
| Public surface | `authhttp.WithChallenges`, `authhttp.NewMiddleware`, and HTTP status/header behavior; the listed `authhttp` APIs have canonical equivalents owned by `adapters/http` (`authenticationhttp`) and remain available through the deprecated alias facade. |
