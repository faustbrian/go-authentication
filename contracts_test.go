...
func TestContextCarriesPrincipalWithPrivateKey(t *testing.T) {
	...
	ctx := context.WithValue(context.Background(), "principal", "collision") //nolint:staticcheck // proves string keys cannot collide.
	...
	if got := ctx.Value("principal"); got != "collision" { //nolint:staticcheck // proves string keys cannot collide.
