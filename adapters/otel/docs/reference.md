...
`BenchmarkAuthenticationInstrumentation` reports latency, throughput, and allocations for
...
### Why does construction return an invalid-configuration error?

Both providers must be non-nil, and their meter, tracer, and instrument
construction methods must return without panicking. The adapter deliberately
returns fixed public errors instead of exposing provider details.

### Why are no signals exported?

Check the caller-owned SDK's sampling, readers, processors, exporters, and
shutdown order. This adapter never consults global providers and cannot report
exporter failures through authentication results.

### Why is authentication completion delayed?

The supplied provider is executing observer work synchronously. Use no-op,
