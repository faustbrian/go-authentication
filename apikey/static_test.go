...
func TestZeroValueStaticCanInstallInitialKeys(t *testing.T) {
	t.Parallel()

	var authenticator apikey.Static
	if err := authenticator.Replace([]apikey.Entry{{
		ID: "initial", Key: "initial-secret",
	...
	if _, err := authenticator.Authenticate(
		context.Background(),
		authentication.NewAPIKeyCredential("initial", "initial-secret"),
	); err != nil {
		t.Fatalf("Authenticate() error = %v", err)
	}
}

func TestStaticRemainsComparable(t *testing.T) {
	t.Parallel()

	values := map[apikey.Static]struct{}{apikey.Static{}: {}}
	if len(values) != 1 {
		t.Fatalf("comparable Static map length = %d, want 1", len(values))
	}
}
