...
| Issue | Neither JOSE nor HTTP defines cache goroutine ownership, overlapping refreshes, constructor-context lifetime, waiter cancellation, or shutdown behavior for a Go provider. |
| Credible interpretations | Let every miss fetch; overlap automatic and explicit refreshes; tie lifetime to constructor context; make shutdown fire-and-forget; let concurrent callers inherit one caller's result; or serialize remote work under an owned lifecycle with caller-specific cancellation. |
...
| Selected behavior | Initial registration is bounded. Automatic and explicit refreshes share one hardened serialized transport; overlapping explicit refreshes share one result. The successful constructor context does not own provider lifetime. Once `Shutdown` begins, it permanently rejects new work, cancels admitted operations, joins them, and shuts down cache goroutines. Each concurrent shutdown caller is bounded by its own context; after an active attempt fails or is canceled, a waiting caller with a live context continues the remaining cleanup instead of inheriting the prior caller's result. `Close` delegates to `Shutdown` for compatibility. |
...
| Compatibility and wire consequences | No JWT or HTTP wire bytes change. Callers must explicitly shut down providers and must not assume canceling the constructor context destroys a successfully returned provider. Deprecated `Close` remains an equivalent delegation throughout the v1 compatibility interval. |
| Executable evidence | `TestRemoteRefreshWaitersShareResultAndHonorCancellation`, `TestRemoteSerializesAutomaticAndExplicitRefreshWork`, `TestRemoteLifetimeIsOwnedByShutdown`, `TestRemoteShutdownOwnsCompleteBoundedCleanup`, `TestRemoteConcurrentShutdownCallerContinuesCleanupAfterPriorCallerCancellation`, `TestRemoteShutdownReportsCanceledJoin`, and `TestRemoteShutdownDeadlineIsNotBlockedByRefreshLock` |
| Public surface | `NewRemote`, `Remote.Refresh`, `Remote.KeySet`, `Remote.Shutdown`, and deprecated `Remote.Close` |
